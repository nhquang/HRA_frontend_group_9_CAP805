...
export default {
  ...
  data() {
    return {
      fname: '', lname: '', gender: '',username:'', role: '', hireDate: '', email: '',streetAddress: '',city: '',province:'',country:'',phone:'',stillEmployed:'',branchId: '', deparmentId:'',payInfo:{ frequency:'',bankAccountNumber:'',firstPayDate:'',salary:'',grossAmount:''},
      ...
      if(confirm) { 
        ...
        if(this.validation.errors.length == 0) {
          try {
            ...
            var data = {
              fname:this.fname, lname: this.lname, gender: this.gender,username:this.username, role: this.role, hireDate: this.hireDate, email: this.email,streetAddress: this.streetAddress,city: this.city,province:this.province,country:this.country,phone:this.phone,stillEmployed:this.stillEmployed,branchId: this.branchId, deparmentId:this.deparmentId,payInfo:{ frequency:this.payInfo.frequency,bankAccountNumber:this.payInfo.bankAccountNumber,firstPayDate:this.payInfo.firstPayDate,salary:this.payInfo.salary,grossAmount:this.payInfo.grossAmount}
            }
            var response = await axios.post(this.$store.state.apiUrl+'/employees/add_employee', data, {
               headers: {
                Authorization: 'Bearer '+ this.$cookies.get('_t_hrm')
              }
            });
            if(response.hasOwnProperty('data')) { 
              this.activationCode='';this.fname='';this.lname='';this.gender='';this.username='';this.role='';this.hireDate='';this.email='';this.streetAddress='';this.city='';this.province='';this.country='';this.phone='';this.stillEmployed='';this.deparmentId='';this.branchId='';this.payInfo.frequency='';this.payInfo.bankAccountNumber='';this.payInfo.firstPayDate='';this.payInfo.salary='';this.payInfo.grossAmount='';
            }
